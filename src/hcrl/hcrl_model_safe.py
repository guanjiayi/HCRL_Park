...
class HCRL_Model_Safe(nn.Module):
    '''
    ...
    def __init__(
        ...
        super(HCRL_Model_Safe, self).__init__()
        ...
        if self.share_encoder:
            ...
            self.critic_cost_encoder = FCEncoder(
            obs_shape = obs_shape,
            hidden_size_list = encoder_hidden_size_list,
            activation = activation,
            )
        ...
        self.critic_cost_head = RegressionHead(
            input_size = encoder_hidden_size_list[-1],
            output_size = 1,
            layer_num = critic_head_layer_num,
            activation = activation,
        ...
        if self.share_encoder:
            ...
            self.critic_cost = [self.encoder, self.critic_cost_head]
        else:
            self.actor = [self.actor_encoder, self.actor_head]
            self.critic = [self.critic_encoder, self.critic_head]
            self.critic_cost = [self.critic_cost_encoder, self.critic_cost_head]
        self.actor = nn.ModuleList(self.actor)
        self.critic = nn.ModuleList(self.critic)
        self.critic_cost = nn.ModuleList(self.critic_cost)
    ...
    def compute_critic(self, x: torch.Tensor) -> Dict:
        ...
        if self.share_encoder:
            critic_embedding = self.encoder(x)
        else:
            critic_embedding = self.critic_encoder(x)
            critic_cost_embedding = self.critic_cost_encoder(x)

        value = self.critic_head(critic_embedding)
        value_cost = self.critic_cost_head(critic_cost_embedding)
        return {'value': value['pred'],
                'value_cost': value_cost['pred']}
    
    def compute_actor_critic(self, x: torch.Tensor) -> Dict:

        if self.share_encoder:
            actor_embedding = critic_embedding = critic_cost_embedding = self.encoder(x)
        else:
            actor_embedding = self.actor_encoder(x)
            critic_embedding = self.critic_encoder(x)
            critic_cost_embedding = self.critic_cost_encoder(x)
        ...
        value_cost = self.critic_cost_head(critic_cost_embedding)['pred']
