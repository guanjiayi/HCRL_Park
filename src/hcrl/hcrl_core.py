...
class Actor(nn.Module):
    ...
    def forward(self, x):
        # Shared part - Here, there is only one l1, which is used for both discrete and continuous actions simultaneously.
        x = F.relu(self.l1(x))
        x = F.relu(self.l2(x))
        # discrete
...
class HCRL(object):
    ...
    def select_action(self, state, is_test=False):
        ...
        # pi = dist.sample()
        # Discrete softmax and then select from the distribution.
        discrete_action = Categorical(discrete_prob)
        discrete_action = discrete_action.sample().item()
        # Continuous mean-variance distribution
        # dist = distributions.Normal(mu, std)
        # pi = dist.sample()
    ...
    def train(self, replay_buffer, batch_size=32, c_epoch=10, a_epoch=10):
        ...
        for ae in range(a_epoch):
            ...
            # The discrete part and the continuous part of the loss are added together.
